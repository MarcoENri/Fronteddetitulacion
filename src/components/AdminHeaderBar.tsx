...
// Interfaces (Mantenidas según tu código)
...
export default function AdminHeaderBar({
  ...
  onReloadPeriods,
  careerStats,
  onOpenSidebar, // Inyectamos la función
}: Props) {
  ...
  const whiteBtn = {
    ...
    display: { xs: "none", md: "flex" } // Ocultar en móviles para no saturar
  };
    ...
    <>
      ...
      >
        ...
        <IconButton 
          ...
          sx={{ color: "#fff", mr: 1, bgcolor: "rgba(255,255,255,0.1)" }}
        >
          <MenuIcon />
        </IconButton>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, flex: 1, justifyContent: "center" }}>
          <Box sx={{ display: { xs: "none", lg: "block" }, textAlign: "right" }}>
            ...
            <Typography variant="body2" sx={{ color: "#fff", fontWeight: 700 }}>
              {activePeriod.loading ? "Cargando..." : activePeriod.periodName ?? "SIN PERIODO"}
            </Typography>
          </Box>

          <FormControl size="small" sx={{ minWidth: 180 }}>
            <InputLabel id="period-label" sx={{ color: "#fff", opacity: 0.8, "&.Mui-focused": { color: "#fff" } }}>
              Filtro de período
            </InputLabel>
            <Select
            ...
            >
              ...
              {periods.map((p) => (
                <MenuItem key={p.id} value={p.id}>
                  {p.name} {p.isActive ? " (Actual)" : ""}
                </MenuItem>
              ))}
            </Select>
          </FormControl>

          <IconButton onClick={onReloadPeriods} size="small" sx={{ color: "#fff", border: "1px solid rgba(255,255,255,0.3)" }}>
        ...
        <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
          ...
          <IconButton onClick={onRefresh} sx={{ color: "#fff" }} title="Refrescar Datos">
            <RefreshRoundedIcon />
          </IconButton>
          ...
          <IconButton 
            onClick={onOpenMenu} 
            sx={{ 
              bgcolor: openUserMenu ? "rgba(255,255,255,0.2)" : "transparent",
              color: "#fff", 
              transition: "0.3s" 
            }}
          >
        ...
        >
          ...
          <MenuItem component="label">
            ...
            <input type="file" hidden accept=".xlsx" onChange={(e) => {
          ...
          <MenuItem onClick={() => { onCloseMenu(); setOpenStatsModal(true); }}>
            <ListItemIcon><AssessmentRoundedIcon fontSize="small" sx={{ color: verde }} /></ListItemIcon>
            <Box>
              <Typography variant="body2" fontWeight={700}>Estadísticas</Typography>
              <Typography variant="caption" color="text.secondary">Reporte de rendimiento</Typography>
            </Box>
          </MenuItem>

          <MuiDivider />

          <MenuItem onClick={() => { onCloseMenu(); onLogout(); }} sx={{ color: "error.main" }}>
            <ListItemIcon><ExitToAppRoundedIcon fontSize="small" color="error" /></ListItemIcon>
            Cerrar Sesión
          </MenuItem>
        </Menu>
      </Box>
      ...
      <Dialog open={openStatsModal} onClose={() => setOpenStatsModal(false)} maxWidth="md" fullWidth>
                ...
                {careerStats.map((stat) => (
                  <TableRow key={stat.key} hover>
                    ...
                    <TableCell align="center">
                      <Chip label={stat.total} size="small" sx={{ fontWeight: 900, bgcolor: "#E8F5E9", color: "#2E7D32" }} />
                    </TableCell>
                    <TableCell align="center">
                      <Chip label={stat.reprobados} size="small" sx={{ fontWeight: 900, bgcolor: "#FFEBEE", color: "#C62828" }} />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setOpenStatsModal(false)} variant="outlined" sx={{ fontWeight: 700 }}>
            Entendido
          </Button>
        </DialogActions>
      </Dialog>
