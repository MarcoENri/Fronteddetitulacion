...
export default function LoginPage() {
  ...
  const handleSendToken = async () => {
    ...
    try {
      ...
      setResetStep("reset"); // Avanzar al siguiente paso
    } catch (e: any) {
    ...
    <Box sx={{ minHeight: "100vh", position: "relative", overflow: "hidden", backgroundColor: "#000" }}>
      ...
      <Dialog open={openReset} onClose={closeResetModal} maxWidth="xs" fullWidth>
        ...
        <DialogContent dividers>
          {resetStep === "email" ? (
            <TextField label="Correo Institucional" fullWidth value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} sx={{ mt: 1 }} />
          ) : (
            <Box sx={{ display: "grid", gap: 2, mt: 1 }}>
              <TextField label="Token del correo" fullWidth value={resetToken} onChange={(e) => setResetToken(e.target.value)} />
              <TextField label="Nueva Contraseña" type="password" fullWidth value={resetNewPass} onChange={(e) => setResetNewPass(e.target.value)} />
            </Box>
          )}
          {resetMsg && <Typography sx={{ fontSize: "0.8rem", mt: 1, color: resetMsg.includes("✅") ? "green" : "red" }}>{resetMsg}</Typography>}
        </DialogContent>
        <DialogActions>
          <Button onClick={closeResetModal}>Cancelar</Button>
          <Button variant="contained" onClick={resetStep === "email" ? handleSendToken : handleDoReset} sx={{ bgcolor: brand.primary }}>
