...
export default function CoordinatorStudentDetailPage() {
  ...
  const load = async () => {
    ...
    if (!pid) {
      alert("No hay período activo. Pide al administrador que lo active.");
  ...
  const openAssignModal = async () => {
    ...
    try {
      ...
      alert(e?.response?.data?.message ?? "No se pudo cargar tutores");
    }
  };
  ...
  const handleCreateIncident = async () => {
    ...
    try {
      await createIncident(id, periodId, {
        stage: incStage,
  ...
  const handleCreateObservation = async () => {
    ...
    try {
      await createObservation(id, periodId, { text: obsText });
  ...
  const handleAssignProject = async () => {
    const pid = periodId || Number(localStorage.getItem("periodId"));
    if (!id || !pid || !projectName.trim() || !selectedTutorId) {
      alert("Por favor completa todos los campos");
      return;
    }
    ...
    try {
      await assignProject(id, pid, {
        ...
        tutorId: selectedTutorId as number,
      });
      alert("Asignado correctamente");
      setAssignOpen(false);
      load();
    } catch (e: any) {
      alert(e?.response?.data?.message ?? "No se pudo asignar");
    } finally {
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box sx={{ minHeight: "100vh", background: "#f0f2f5", display: "flex", flexDirection: "column" }}>
              ...
              <CardContent sx={{ p: 3 }}>
                ...
                >
                  ...
                  >
                    ...
                    <Typography variant="body1" sx={{ fontWeight: 600, color: "#212529", wordBreak: "break-word" }}>
                      {data.email}
                    </Typography>
                  </Paper>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.2 }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography
                        ...
                        sx={{
                          fontWeight: 600,
                          color: coordinatorInfo?.username ? "#212529" : "#adb5bd",
                          fontSize: "0.813rem",
                        }}
                      >
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.2 }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography
                        variant="body2"
                        sx={{ fontWeight: 600, color: coordinatorInfo?.name ? "#212529" : "#adb5bd", fontSize: "0.813rem" }}
                      >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.2 }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography
                        ...
                        sx={{
                          fontWeight: 600,
                          color: coordinatorInfo?.email ? "#212529" : "#adb5bd",
                          fontSize: "0.813rem",
        ...
        <Dialog open={assignOpen} onClose={() => setAssignOpen(false)} maxWidth="sm" fullWidth>
          <DialogTitle sx={{ fontWeight: 900, color: VERDE_INSTITUCIONAL }}>Asignar Proyecto y Tutor</DialogTitle>
          <DialogContent>
          ...
          <DialogActions sx={{ p: 2 }}>
            <Button onClick={() => setAssignOpen(false)} sx={{ borderRadius: "20px", textTransform: "none" }}>
              Cancelar
            </Button>
            <Button
              variant="contained"
